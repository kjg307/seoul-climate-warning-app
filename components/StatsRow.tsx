...
const items = (d: Decade) => [
  { icon: "🌡️", label: "기온 상승",   value: `+${d.tempRise}도C`,      sub: "1990년 대비" },
  { icon: "☀️", label: "폭염 일수",   value: `${d.heatDays}일`,         sub: "최고기온 33도 이상" },
  { icon: "🌙", label: "열대야",      value: `${d.tropicalNights}일`,    sub: "최저기온 25도 이상" },
  { icon: "🌊", label: "해수면 상승", value: `+${d.seaLevel}cm`,         sub: "현재 대비" }
];

export default function StatsRow({ decade }: { decade: Decade }) {
  return (
